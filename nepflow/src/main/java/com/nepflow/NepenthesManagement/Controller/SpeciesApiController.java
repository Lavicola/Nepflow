...
public class SpeciesApiController implements SpeciesApi {

    private final SpeciesApiDelegate delegate;

    public SpeciesApiController(@Autowired(required = false) SpeciesApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new SpeciesApiDelegate() {});
    }

    @Override
    public SpeciesApiDelegate getDelegate() {
        return delegate;
    }
